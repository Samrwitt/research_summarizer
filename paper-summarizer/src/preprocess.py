...
def _chunk_text(text, chunk_size=3000, overlap=200):
   """
   Chunk text by approximating tokens (1 char ~ 0.25 token? No, 1 token ~ 4 chars).
   HF models usually max 1024 tokens. 3000 chars is safe-ish for input to a summarizer that outputs smaller.
   Actually T5/BART take typically 1024 tokens input. 
   1024 tokens * 4 chars/token = 4096 chars.
   Let's be safe with 3500 chars.
   """
